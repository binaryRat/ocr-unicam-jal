...
def custom_model_machine_written(image):
    ...
    if reader is None:
        reader = easyocr.Reader(['it'], recog_network='modello_macchina',
                                model_storage_directory="models/machine-written-model/",
                                user_network_directory="models/machine-written-model/", cudnn_benchmark=True, gpu=True)
    return compute_ocr(image)
...
def custom_model_hand_written(image):
    ...
    if reader is None:
        reader = easyocr.Reader(['it'], recog_network='modello_mano',
                                model_storage_directory="models/hand-written-model/",
                                user_network_directory="models/hand-written-model/", cudnn_benchmark=True, gpu=True)
    return compute_ocr(image)
